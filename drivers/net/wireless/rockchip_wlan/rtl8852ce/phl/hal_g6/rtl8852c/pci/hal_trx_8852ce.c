 ...
 */
static u8 hal_mapping_hw_tx_chnl_8852ce(struct hal_info_t *hal,
				u16 macid, enum rtw_phl_ring_cat cat, u8 band)
{
	...
	if (false == is_dbcc_sup(hal->phl_com))
		goto non_dbcc;
	if (0 == band) {
		switch (cat) {
...
static void hal_query_txch_map_8852ce(enum phl_band_idx band, void *ch_map)
{
...
static u8 hal_query_txch_hwband_8852ce(u8 dma_ch)
{
	u8 band = 0;
	switch (dma_ch) {
	case ACH0_QUEUE_IDX_8852CE:
						...
						struct rtw_phl_pkt_req *req, u32 *wd_len)
{
	...
	dma_ch = hal_mapping_hw_tx_chnl_8852ce(hal, mdata->macid,
						mdata->cat,
						mdata->band);
	...
	if (req->wd_seq_offset == 0) {
		...
		w0 = le32_to_cpu(wd_words[0])
				 & ~((AX_TXD_HW_SSN_SEL_MSK << AX_TXD_HW_SSN_SEL_SH)
					 ...
					 | AX_TXD_HWAMSDU
					 | (AX_TXD_HDR_LLC_LEN_MSK << AX_TXD_HDR_LLC_LEN_SH)
#endif
					 );
				...
				struct rtw_phl_pkt_req *req)
{
	...
	do {
		...
		if (true == tx_req->mdata.hw_amsdu) {
			if (0 == tx_req->mdata.hw_seq_mode) {
				PHL_ERR("[8822CE] HW A-MSDU depends on HW Sequence enable, reset hw_amsdu settings\n");
				tx_req->mdata.hw_amsdu = false;
				tx_req->mdata.wdinfo_en = true;
			} else {
				/* reset wdinfo_en */
				tx_req->mdata.wdinfo_en = false;
			}
		}
...
#endif
#ifdef CONFIG_DBCC_SUPPORT
		if (is_dbcc_sup(hal->phl_com)) {
			wp_seq = (tx_req->mdata.band == HW_BAND_1) ?
				(req->wp_seq | WP_BAND_INDIC_RESERVED_BIT) :
				req->wp_seq;
		} else
#endif /* CONFIG_DBCC_SUPPORT */
		{
				...
				u8 *macid, u8* ac_queue, u8 *txsts)
{
	...
	do {
...
#ifdef CONFIG_DBCC_SUPPORT
		if (is_dbcc_sup(hal->phl_com)) {
			band = (*wp_seq & WP_BAND_INDIC_RESERVED_BIT) ?
				HW_BAND_1 : HW_BAND_0;
		} else
#endif /* CONFIG_DBCC_SUPPORT */
		{
		...
		*dma_ch = hal_mapping_hw_tx_chnl_8852ce(hal, *macid, tid, band);
...
void hal_trx_ops_init_8852ce(void)
{
	...
	ops.query_txch_map = hal_query_txch_map_8852ce;
