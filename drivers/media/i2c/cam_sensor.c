...
static int imx219_enum_frame_sizes(struct v4l2_subdev *sd,
	...
	struct v4l2_subdev_frame_size_enum *fse)
{
	if (fse->index >= ARRAY_SIZE(imx219_supported_modes))
		return -EINVAL;
			...
			const struct i2c_device_id *id)
{
	...
	struct gpio_desc *enable_gpio, *power_gpio, *clksel_gpio;
	struct pinctrl *pinctrl;
	struct pinctrl_state *pins_default;
	...
	ret = of_property_read_u32(np, "i2c-index", &i2c_index);
	if (ret) {
		dev_err(dev, "could not get i2c index!\n");
		return -EINVAL;
	}
	...
	power_gpio = devm_gpiod_get(dev, "power", GPIOD_OUT_HIGH);
	if (IS_ERR(power_gpio))
		dev_warn(dev, "Failed to get power_gpios\n");

	clksel_gpio = devm_gpiod_get(dev, "clksel", GPIOD_OUT_HIGH);
	if (IS_ERR(clksel_gpio))
	...
	if (ret < 0) {
		...
		if(ret < 0) return ret;
		isOV5647[i2c_index] = 0;
	} else {
		isOV5647[i2c_index] = 1;
	}

	if(isOV5647[i2c_index]) {
		...
		sensor->enable_gpio = enable_gpio;
		sensor->power_gpio = power_gpio;
		sensor->clksel_gpio = clksel_gpio;
		sensor->module_index = i2c_index;
		sensor->module_facing = "back";
		sensor->module_name = "RPi_v13";
		sensor->len_name = "default";
...
mutex_remove:
		...
		priv->clksel_gpio = clksel_gpio;
		priv->module_index = i2c_index;
