...
int tb_gpios(struct device *dev, int *boardid, int *projectid)
{
	int ret;
	...
	*projectid = pid;
	...
	file = proc_create_single("boardmodel", 0444, NULL, model_show);
	if (!file)
		return -ENOMEM;

	file = proc_create_single("boardid", 0444, NULL, bid_show);
	if (!file)
		return -ENOMEM;

	file = proc_create_single("projectid", 0444, NULL, pid_show);
	if (!file)
		return -ENOMEM;
